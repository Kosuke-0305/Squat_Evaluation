...
| 手法 | AUC | invalid Recall | invalid Precision |
|---|---|---|---|
| ベースライン（全て有効と予測） | 0.500 | 0% | — |
| ルールベース（閾値最適化後） | 0.721 | 56% | 0.71 |
| **本モデル（LSTM + データ拡張）** | **0.731 ± 0.029**（5-fold CV） | ~80% | ~0.62 |

**ルールベース最適閾値詳細（全372件）:**

| クラス | Precision | Recall | F1 |
|---|---|---|---|
| invalid | 0.71 | 0.56 | 0.62 |
| valid | 0.80 | 0.88 | 0.84 |
| **全体 AUC** | | | **0.721** |

最適閾値: depth=110°, lockout=167°, bar_descent=0.010（詳細は下記）

**LSTM 検証セット詳細（74件）:**

| クラス | Precision | Recall | F1 |
|---|---|---|---|
| invalid | 0.62 | 0.80 | 0.70 |
| valid | 0.88 | 0.76 | 0.81 |
| **全体 AUC** | | | **0.780** |
...
```
training_data/
  seq_{ID}.json      # レップごとの特徴量シーケンス（元データ: 372件）
...
無効試技 125件を7手法で拡張し `training_data/` に保存します。
...
閾値最適化実験（`threshold_sweep.py`）により決定した値：

| 失敗要素 | 閾値 | 判定条件 |
|---|---|---|
| 深さ不足 | ≤ **110°** | シーケンス中に代表膝角度をこの値以下まで曲げなければ無効 |
| ロックアウト不足 | ≥ **167°** | 開始・終了付近3フレームで代表膝角度がこの値以上でなければ無効 |
| バー下降 | ≥ **0.010** | 上昇フェーズで肩 Y delta が前フレームより増加すれば無効 |
| 反復動作 | ≥ 10° × 2回 | 下降フェーズで股関節角度の方向逆転が2回以上で無効 |
| 足ずれ | ≥ 0.05 | 足首 X delta の絶対値がこの値以上のフレームがあれば無効 |

閾値は `classifier.py` の `THRESHOLDS` で変更できます。
最適化スクリプト: `python threshold_sweep.py --data-dir training_data`
