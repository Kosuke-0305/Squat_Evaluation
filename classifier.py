...
THRESHOLDS = {
    "depth":        110.0,  # 代表膝角度がこの値以下なら深さ達成（度）
    "lockout":      167.0,  # 代表膝角度がこの値以上でロックアウト達成（度）
    "bar_descent":   0.010, # 肩 Y delta の前フレーム比増加でバー下降検出（正規化座標）
...
def _repr_knee(f: dict) -> float | None:
    """代表膝角度を返す（左右平均、片側 0 の場合は有効側のみ）。
    両側とも未検出（0）の場合は None を返す。"""
...
def classify_attempt(
    ...
    # 未検出フレーム（None）を除いた角度リスト
    knee_angles_nz = [ka for ka in knee_angles if ka is not None]

    # 最深点フレームを特定（未検出フレームを除外してインデックスを探す）
    if valley_frame_idx is None or not (0 <= valley_frame_idx < n):
        if knee_angles_nz:
            valid_idxs = [i for i, ka in enumerate(knee_angles) if ka is not None]
            valley_idx = min(valid_idxs, key=lambda i: knee_angles[i])
        else:
            valley_idx = n // 2
    else:
    ...
    # ---- depth_score ----
    if knee_angles_nz:
        depth_score = 3 if not any(ka <= THRESHOLDS["depth"] for ka in knee_angles_nz) else 0
    else:
        depth_score = 0  # 関節が全フレーム未検出 → 判定しない

    # ---- lockout_score ----
    # 先頭3フレームと末尾3フレームのいずれかが lockout 閾値を達成しているかチェック
    start_frames = [ka for ka in knee_angles[:min(3, n)] if ka is not None]
    end_frames   = [ka for ka in knee_angles[max(0, n - 3):] if ka is not None]
    lockout_ok   = (
        bool(start_frames) and any(ka >= THRESHOLDS["lockout"] for ka in start_frames) and
        bool(end_frames)   and any(ka >= THRESHOLDS["lockout"] for ka in end_frames)
    )
