...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--crop-filter",         action="store_true",       help="フレームを横方向にクロップしてからMediaPipeに渡す。補助員が端に位置する動画での誤検出を抑制する")
    parser.add_argument("--crop-min",           type=float, default=None,  metavar="X", help="クロップの左端（正規化X座標 0.0〜1.0、デフォルト: 0.25）")
    parser.add_argument("--crop-max",           type=float, default=None,  metavar="X", help="クロップの右端（正規化X座標 0.0〜1.0、デフォルト: 0.75）")
    parser.add_argument("--smooth-ankle",        action="store_true",       help="足首X座標に急峻変化抑制＋EMAスムージングを適用する。補助員との揺れ挙動が多い場合に有効にする。")
    parser.add_argument("--ankle-jump",          type=float, default=None,  metavar="X", help="足首X座標の1フレームあたりの変化がこの値以上なら異常とみなす（正規化座標 0.0〜1.0、デフォルト: 0.08）")
    parser.add_argument("--ankle-alpha",         type=float, default=None,  metavar="A", help="EMAの重み係数（0.0〜1.0、大きいほど現在値の影響が強い、デフォルト: 0.2）")
...
def main() -> None:
    ...
    estimator = PoseEstimator(
        ...
        smooth_ankle=args.smooth_ankle,
        ankle_jump=args.ankle_jump,
        ankle_alpha=args.ankle_alpha,
    )
    ...
    if args.smooth_ankle:
        estimator.reset_ankle_smoothing()
